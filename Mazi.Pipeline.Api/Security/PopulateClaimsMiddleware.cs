...
using Mazi.Pipeline.JsonUtilities;
...
public class PopulateClaimsMiddleware : IMiddleware
{
   ...
   private void ProcessNonDevelopmentModeClaims(HttpContext context, List<Claim> claims)
   {
   ...
   private void ProcessDevelopmentModeClaims(HttpContext context, List<Claim> claims)
   {
      if (
         context.User != null
   ...
   {
      AddClaimsFromDatabaseAndCreateUserIfNotPresent(claims, info.Username);
   }
   ...
   private static bool AddClaimIfExists(
      List<Claim> claims,
      JsonEditor editor,
   ...
   {
      var identityProviderHeader = GetHeaderValue(
         context,
         SecurityConstants.Claim_X_MsClientPrincipalIdp
      );

      var username = GetHeaderValue(
         context,
         SecurityConstants.Claim_X_MsClientPrincipalName
      );

      if (identityProviderHeader != null && username != null)
      {
   ...
   )
   {
      ...
      if (user == null || user.Claims == null)
      {
         throw new InvalidOperationException(
            "User or user claims collection was null."
         );
      }
      ...
      foreach (var item in values)
      {
         ...
         {
            AddClaim(claims, ClaimTypes.Role, item.ClaimValue);
         }
         else
         {
            AddClaim(claims, item.ClaimName, item.ClaimValue);
         }
      }
   ...
   private static bool AddClaimsFromHeader(HttpContext context, List<Claim> claims)
   {
      ...
      {
         var identityHeader = GetHeaderValue(
            context,
            SecurityConstants.Claim_X_MsClientPrincipalId
         );

         var nameHeader = GetHeaderValue(
            context,
            SecurityConstants.Claim_X_MsClientPrincipalName
         );
         ...
         claims.Add(
            new Claim(SecurityConstants.Claim_X_MsClientPrincipalId, identityHeader)
         );

         claims.Add(
            new Claim(SecurityConstants.Claim_X_MsClientPrincipalName, nameHeader)
         );
