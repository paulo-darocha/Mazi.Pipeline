...
public class AzureEasyAuthClient
{
   ...
   public bool IsReadyForAuthenticatedCall { get; private set; }
   ...
   private void TryInitializeHttpClientUsingSessionCookie(HttpRequest request)
   {
      ...
      if (
         requestCookies.ContainsKey(SecurityConstants.Cookie_AppServiceAuthSession)
         == false
      )
      {
      ...
      else
      {
         ...
         var authCookie = requestCookies[
            SecurityConstants.Cookie_AppServiceAuthSession
         ];

         container.Add(
            new Uri(baseUrl),
            new Cookie(SecurityConstants.Cookie_AppServiceAuthSession, authCookie)
         );
