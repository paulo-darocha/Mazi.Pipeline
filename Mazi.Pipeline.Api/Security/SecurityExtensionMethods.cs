...
public static class SecurityExtensionMethods
{
   ...
   public static bool ContainsClaim(this IEnumerable<Claim> claims, string claimName)
   {
      var match = claims.GetClaim(claimName);
      if (match == null)
         return false;
   ...
   {
      var match = claims
         .Where(x => x.Type == ClaimTypes.Role && x.Value == roleName)
         .FirstOrDefault();
   ...
   public static string GetHeaderValue(this IHeaderDictionary headers, string name)
   {
